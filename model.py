...
class o2mCycleGAN():
    def __init__(self, atts=None, data_dir='./data'):
        ...
        self.d_T_lst = [self.__discriminator__() for _ in atts]

        # fake -> truth discriminators
        self.d_F_lst = [self.__discriminator__() for _ in atts]
        ...
        class_gen_F_lst = [d_F(gen_T) for (d_F, gen_F) in zip(self.d_F_lst, gen_F_lst)]
    ...
    def build_data(self, method='train'):
        data_loder = CelebDataLoader(self.data_dir, self.atts, method=method)
        data_it = data_laoder.__iterator__()
        _img, _label = it.get_next()
        session = tf.Session()
        ...
        for epoch in range(epochs):
            for label in data.keys():
                ...
                for fake_label in range(self.n_atts):
                    if fake_labe == label: continue
                    img_B += data[fake_label]
